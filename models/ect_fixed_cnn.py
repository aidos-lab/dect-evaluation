...
from models.layers.fixedlayers import EctEdgesLayer, EctPointsLayer
...
class ECTCNNPointsModel(BaseModel):
    def __init__(self, config):
        ...
        self.linear1 = nn.Sequential(
            nn.Dropout(), nn.Linear(num_features, config.hidden), nn.ReLU()
        )
        self.linear2 = nn.Sequential(
            nn.Dropout(), nn.Linear(config.hidden, config.hidden // 2), nn.ReLU()
        )
        self.linear3 = nn.Sequential(
            nn.Dropout(), nn.Linear(config.hidden // 2, config.num_classes)
        )
