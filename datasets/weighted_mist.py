...
from datasets.transforms import WeightedMnistTransform

from loaders.factory import register


class WeightedMnistDataModule(DataModule):
    def __init__(self, config):
        self.config = config
        self.transform = transforms.Compose(
            [WeightedMnistTransform(), FaceToEdge(remove_faces=False), CenterTransform()]
        )
        super().__init__(
...
class WeightedMnistDataset(InMemoryDataset):
    """
    This generates the Weighted Complexes for the WECT. 
    """
    def __init__(
...
def initialize():
    register("dataset", WeightedMnistDataModule)
